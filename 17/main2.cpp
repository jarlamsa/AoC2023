...
int main()
{
	...
	int distance[MAP_SIZE][MAP_SIZE][5][MAX_MOVE+1];
	bool visited[MAP_SIZE][MAP_SIZE][5][MAX_MOVE+1] = {false};
	...
	while (!Q.empty()) {
		...
		if (n.row > 0 && n.direction != 3) {
			if ((n.direction == 1 && n.moved < MAX_MOVE) || (n.direction != 1 && n.moved >= MIN_MOVE) || n.direction == 0) {
				if (n.direction == 1) {
					moved = n.moved + 1;
				} else {
					moved = 1;
		...
		if (n.col > 0 && n.direction != 4) {
			if ((n.direction == 2 && n.moved < MAX_MOVE) || (n.direction != 2 && n.moved >= MIN_MOVE) || n.direction == 0) {
				if (n.direction == 2) {
					moved = n.moved + 1;
				} else {
					moved = 1;
		...
		if (n.row < MAP_SIZE - 1 && n.direction != 1) {
			if ((n.direction == 3 && n.moved < MAX_MOVE) || (n.direction != 3 && n.moved >= MIN_MOVE) || n.direction == 0) {
				if (n.direction == 3) {
					moved = n.moved + 1;
				} else {
					moved = 1;
		...
		if (n.col < MAP_SIZE - 1 && n.direction != 2) {
			if ((n.direction == 4 && n.moved < MAX_MOVE) || (n.direction != 4 && n.moved >= MIN_MOVE) || n.direction == 0) {
				if (n.direction == 4) {
					moved = n.moved + 1;
				} else {
					moved = 1;
