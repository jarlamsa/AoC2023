...
// Count the energized tiles and reset them
...
int main()
{
    ...
    for (int i = 0; i < map.at(0).size(); i++) {
        followBeam(map, energized, 3, 0, i);
        uint32_t sum = countEnergized(energized);
        max_sum = max(max_sum, sum);
    }
    //Left column
    for (int i = 0; i < map.size(); i++) {
        followBeam(map, energized, 4, i, 0);
        uint32_t sum = countEnergized(energized);
        max_sum = max(max_sum, sum);
    }
    //Bottom row
    for (int i = 0; i < map.at(0).size(); i++) {
        followBeam(map, energized, 1, map.size() - 1, i);
        uint32_t sum = countEnergized(energized);
        max_sum = max(max_sum, sum);
    }
    //Right column
    for (int i = 0; i < map.size(); i++) {
        followBeam(map, energized, 2, i, map.at(0).size() - 1);
        uint32_t sum = countEnergized(energized);
        max_sum = max(max_sum, sum);
    }
    ...
    cout << "Maximum number of energized tiles " << max_sum << " time taken " << duration.count() << " us" << endl;
